...
export const createCollection = async (collectionName) => {
  ...
  try {
    ...
    throw new CustomError(
      `Bunny Collection Error: ${error.response?.data?.message || error.message}`,
      500,
    );
  }
};
...
export const createVideoEntry = async (title, collectionId) => {
  ...
  try {
    ...
    throw new CustomError(
      `Bunny Video Error: ${error.response?.data?.message || error.message}`,
      500,
    );
  }
};
export const deleteVideoEntry = async (bunnyVideoId) => {
  ...
  try {
    ...
    // If video is not found (404), consider it already deleted
